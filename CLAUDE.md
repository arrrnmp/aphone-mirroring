...
1. **macOS app** (`macos/`) — A native macOS SwiftUI application that mirrors Android device screens over USB using the scrcpy protocol (v3.3.4). Handles H.264 video streaming, PCM audio streaming, hardware-accelerated decoding, and full input forwarding (keyboard, mouse, touch, scroll, gesture simulation).

2. **Android companion app** (`android/`) — A foreground TCP service that exposes real phone data (SMS, calls, photos, contacts, notifications, active call state) to the macOS app over a local USB connection via `adb forward`. Does **not** handle screen mirroring — that is managed entirely by the scrcpy protocol on the macOS side.
...
Open `macos/aPhone Mirroring.xcodeproj` in Xcode, or build via CLI:
...
Open `android/` in Android Studio (Electric Eel or later), or via CLI:
...
All source files are in `android/app/src/main/java/com/aaronmompie/phoneconnect/`.
...
All declared in `android/app/src/main/AndroidManifest.xml`. Runtime permissions requested in `MainActivity` on launch:
